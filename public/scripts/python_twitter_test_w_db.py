...
def main():
	...
	sql_main_table = """ CREATE TABLE retweeter_data IF NOT EXISTS projects (
	...
	if DB_CONNECTION is not None:
		...
		create_table(conn, sql_main_table)
	else:
	...
	today = date.today()
	tweet_data = [('www.1', 'foobarfoo', today),
				('www.2', 'abdcas', today),
				('www.3', 'asdflkhw', today),
				]
	
	
	pulled_data = get_usertweets_articles("aasdfang", NEWS_SOURCES, CONSERVATIVE, CONSERVATIVE, POOL_METHOD)[2]	
	for i in pulled_data:
		i['date'] = today
	new_list = [(key,)+tuple(val) for pulled_data in list for key,val in pulled_data.items()]


	CURSOR.executemany('''INSERT INTO retweeter_data VALUES (?,?,?)''', tweet_data)


	DB_CONNECTION.commit()


	for row in CURSOR.execute('''SELECT TOP 100 url, user_tweets FROM retweeter_data ORDER BY create_date DESC'''):
		print row
